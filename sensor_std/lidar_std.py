...
class lidar_carla:
            ...
            if npc.id != self.vehicle.id:
                ...
                if dist < 120:
                    ...
                    boudning_box_np[0:3] = rotate_yaw(
                        boudning_box_np[0:3],
                        np.radians(self.lidar.get_transform().rotation.yaw),
                    )
                    boudning_box_np[0:3] = rotate_pitch(
                        boudning_box_np[0:3],
                        np.radians(-self.lidar.get_transform().rotation.pitch),
                    )
    ...
    def _pcs_callback(self, point_cloud) -> None:
        data = np.copy(np.frombuffer(bytes(point_cloud.raw_data), dtype=np.dtype("f4")))
